    ...
    async start() {
        this._server = http.createServer(async (req, res) => {
            ...
            try {
                ...
                // === MITM STABILITY: Fetch errors are non-fatal (ValorantNarrator behavior) ===
                // Log the error but DO NOT crash - allow MITM to continue
                ...
                try {
                    res.writeHead(502)
                    res.write(JSON.stringify({ error: 'Upstream request failed' }))
                    res.end()
                } catch (resErr) {
        ...
        // === MITM STABILITY: HTTP server error handling ===
        this._server.on('error', (err) => {
            console.log(JSON.stringify({
                type: 'error',
                time: Date.now(),
                code: 500,
                reason: `HTTP server error: ${err.message}`
            }) + '\n')
            // DO NOT crash - allow MITM to continue
        })

        this._server.on('clientError', (err, socket) => {
            // Client connection errors are non-fatal
            if (!socket.destroyed) {
                socket.destroy()
            }
        })
