...
| — | Global Crash Logging | Architecture-agnostic crash logging. `%LOCALAPPDATA%\ValVoice\logs\valvoice.log` with daily rotation (7 days). Global `Thread.setDefaultUncaughtExceptionHandler` captures all uncaught exceptions. Logs directory created at startup. `CRASH_LOG` appender added to `logback.xml`. No engine/queue/playback changes | `Main.java`, `logback.xml` |
