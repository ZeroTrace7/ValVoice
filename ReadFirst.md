...
### XTTS Playback Architecture

- `VoiceGenerator.java` owns the XTTS HTTP request, `HttpResponse.BodyHandlers.ofInputStream()` response stream, and `dev.mccue.jlayer.player.advanced.AdvancedPlayer` playback lifecycle.
- XTTS Push-to-Talk is driven by `java.awt.Robot` + `java.awt.event.KeyEvent` inside `VoiceGenerator.java`.
...
├── README.txt                        # End-user guide (bundled in release)
...
└── src/
    ├── assembly/
    │   └── distribution.xml              # Maven assembly descriptor for release ZIP
    └── main/
    ├── java/
    ...
    │       │   ├── EnvironmentValidator.java   # Startup dependency diagnostics
    ...
    │           ├── Main.java
    │           ├── MessageType.java
    │           ├── SettingsController.java       # Settings window controller
    │           ├── SetupWizardController.java    # First-run setup wizard
    ...
    └── resources/
        └── com/someone/valvoicegui/
            ...
            ├── setup-wizard.fxml                 # First-run wizard layout
...
- **Behavior:** Final utility class (private constructor, all static methods). Three checks:
  ...
  - `runChecksWithResults()` — calls the same three check methods but returns `Map<String, Boolean>` with keys `"SoundVolumeView"`, `"PowerShell"`, `"VBCable"`. Used by `SetupWizardController` to display green/red status labels in the wizard UI. Also logs results.
...
- **Fields:**
  ...
  - `firstRunCompleted` (boolean, default `false`) — tracks whether the first-run Setup Wizard has been completed; once `true`, wizard never shows again
...
- **Behavior:** Overrides `Application.start(Stage)`:
  - **Wizard routing:** Checks `ConfigManager.get().firstRunCompleted`. If `false`, calls `launchSetupWizard(stage)` which loads `setup-wizard.fxml` into a transparent `StageStyle.TRANSPARENT` window and returns early. If `true`, calls `launchMainApp(stage)` for normal startup.
...
- `firstRunCompleted` defaults to `false`. Set to `true` by the Setup Wizard on completion. Controls whether the wizard or main UI is shown at startup.
...
Step 0h: If firstRunCompleted == false → launch Setup Wizard (4 pages).
         Wizard validates environment → shows Valorant audio instructions → marks complete.
         Then launches main application window.
Step 0i: If firstRunCompleted == true → load mainApplication.fxml directly.
...
```
Main.java
  ├─► EnvironmentValidator.runAllChecks()     ← Startup diagnostics
  ├─► SystemAudioRouter.routeApplicationAudio()      ← Startup audio hijack
  ├─► ConfigManager.load()                    ← Persistent config
  └─► ValVoiceApplication.java
        │
        ├─► [firstRunCompleted == false] → SetupWizardController.java  ← First-run wizard
        ...
        └─► [firstRunCompleted == true] → ValVoiceController.java
              ├─► SettingsController.java      ← Settings window
...
|----------------------|-----------------|
| MITM Proxy           | 🟢 Stable       |
| TTS Engine Lifecycle | 🟢 Stable       |
| HTTP + Streaming     | 🟢 Stable       |
| Direct SAPI Fallback | 🟢 Stable       |
| PTT (`java.awt.Robot`) | 🟢 Stable    |
| VB-Cable Routing     | 🟢 Stable       |
| Auto Recovery        | 🟢 Stable       |
| Configuration System | 🟢 Stable       |
| Settings UI          | 🟢 Stable       |
| Runtime Config Sync  | 🟢 Stable       |
| Env Validation       | 🟢 Stable       |
| Release Packaging    | 🟢 Stable       |
| Setup Wizard         | 🟢 Stable       |
